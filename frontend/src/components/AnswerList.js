...
const AnswerList = ({ questionId }) => {
	...
	const fetchAnswers = useCallback(async () => {
		try {
			...
			const res = await axios.get(`/api/questions/${questionId}/answers?userId=${currentUser?.id || ""}`);
			setAnswers(res.data || []);
			setError(null);
		} catch (err) {
	...
	const handleRegenerateAnswer = async (answerId, questionId) => {
		try {
			...
			const res = await axios.post(`/api/questions/${questionId}/ai-answer/regenerate`, {
				authorId: currentUser.id,
				answerId: answerId,
			});
			...
			if (res.data.success) {
				...
				setAnswers(answers.map((a) => (a._id === answerId ? res.data.data : a)));
			}
		} catch (err) {
	...
	const handleKeepAnswer = async (answerId, questionId) => {
		try {
			...
			const res = await axios.put(`/api/questions/${questionId}/ai-answer/${answerId}/keep`, {
				authorId: currentUser.id,
			});
							...
							<div className="answer-meta">
								...
								{a.aiGenerated && a.visibleOnlyToAuthor && !a.aiAnswerKept && (
									<div className="ai-answer-actions">
										...
										>
											{regeneratingId === a._id ? "Regenerating..." : "Regenerate"}
										</button>
										<button
