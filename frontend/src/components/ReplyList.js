...
const ReplyList = ({ answerId }) => {
  ...
  const fetchReplies = async () => {
    try {
      ...
      const res = await axios.get(
        `http://localhost:5000/api/questions/answer/${answerId}/replies`
      );
      setReplies(res.data || []);
      setError(null);
    } catch (err) {
  ...
  const submitReply = async (e) => {
    ...
    try {
      setLoading(true);
      await axios.post(
        `http://localhost:5000/api/questions/answer/${answerId}/reply`,
        {
          text: newReply,
          ...
          {replies.map((r) => (
            <div key={r._id} className="reply-item">
              ...
              <div className="reply-meta">
                replied by <strong>{r.authorName || "Anonymous"}</strong> on {new Date(r.createdAt).toLocaleDateString()}
              </div>
            </div>
          ))}
        </div>
