...
const QuestionDetail = ({ questionId, onBackClick }) => {
	...
	const handleGenerateAIAnswer = async () => {
		try {
			...
			const res = await axios.post(`/api/questions/${questionId}/ai-answer`, { authorId: user.id });
			...
			{loading ? (
				...
				<>
					<div className="question-detail-header">
						...
						{question.authorId === JSON.parse(localStorage.getItem("user") || "{}")?.id && (
							<button
								className="ask-ai-btn"
								onClick={handleGenerateAIAnswer}
								disabled={generatingAI || aiAnswerGenerated}
							>
								{generatingAI ? "Generating AI Answer..." : aiAnswerGenerated ? "✓ AI Answer Generated" : "🤖 Ask AI"}
							</button>
						)}
					</div>

					{aiAnswerMessage && <div className={`ai-message ${aiAnswerMessage.type}`}>{aiAnswerMessage.text}</div>}
