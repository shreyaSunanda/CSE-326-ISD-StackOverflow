...
router.post("/:questionId/ai-answer", async (req, res) => {
	try {
		...
		await Question.findByIdAndUpdate(req.params.questionId, { isAiAnswerGenerated: true }, { new: true });
		...
		console.error("AI answer generation error:", err.response?.data || err.message);
		res.status(500).json({
			success: false,
...
router.post("/:questionId/ai-answer/regenerate", async (req, res) => {
	try {
		...
		const systemPrompt = `You are a Stack Overflow expert. Generate a concise, helpful answer to the following question. Keep it professional and informative. Use simple markdown formatting. Generate a DIFFERENT answer than any previous attempts.`;
		...
		const updatedAnswer = await Answer.findByIdAndUpdate(answerId, { text: aiAnswer }, { new: true });
		...
		console.error("AI answer regeneration error:", err.response?.data || err.message);
		res.status(500).json({
			success: false,
...
router.get("/:questionId/answers", async (req, res) => {
	try {
		...
		let answers = await Answer.find({ questionId: req.params.questionId }).sort({ createdAt: 1 });
			...
			axios.post(
				"https://api.groq.com/openai/v1/chat/completions",
				{
					...
					messages: [
						{
							role: "system",
							content: "Polish the following question for grammar, clarity, and conciseness:",
						},
